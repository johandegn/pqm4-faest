...
static void aes_key_schedule_backward_128_vbb_vk_round_unmasked(vbb_t* vbb, uint8_t Mtag, uint8_t Mkey,
...
static void aes_key_schedule_128(const uint8_t* w, vbb_t* vbb,
  ...
  for (unsigned int j = 0; j < Ske; j+=4) {
    // backward 1
    aes_key_schedule_backward_1_round(w + FAEST_128F_LAMBDA / 8, k, w_dash, j, params, true);
    aes_key_schedule_backward_128_vbb_vk_round_unmasked(vbb, 1, 0, NULL, v_w_dash, j);
...
static void aes_enc_forward_backward_128_unmasked(vbb_t* vbb, unsigned int offset, const uint8_t* in, const uint8_t* out,
...
static void aes_constraints_0_128(const uint8_t* in, const uint8_t* out, const uint8_t* w,
  ...
  for (unsigned int j = 0; j < FAEST_128F_R; j++) {
    ...
    // backward 1
    aes_enc_backward_128_1_round(w, k, out, s_dash, j, 1);

    if (j != 0){
      memcpy(vs, vs_old, sizeof(vs));
    }
    aes_enc_forward_backward_128_unmasked(vbb, offset, in, out, 1, 0, NULL, vs, vs_old, vs_dash, j);
    for (unsigned int i = 0; i < 16; i++) {
...
static void aes_key_schedule_constraints_Mkey_1_128_new(vbb_t* vbb, const uint8_t* delta,
  ...
  for (unsigned int j = 0; j < FAEST_128F_Ske; j += 4) {
    aes_key_schedule_backward_128_vbb_vk_round_unmasked(vbb, 0, 1, delta, q_w_dash, j);
...
static void aes_enc_constraints_Mkey_1_128(const uint8_t* in, const uint8_t* out, vbb_t* vbb,
  ...
  for (unsigned int j = 0; j < FAEST_128F_R; j++){
    ...
    */
    ...
    aes_enc_forward_backward_128_unmasked(vbb, offset, in, out, 0, 1, delta, qs, qs_old, qs_dash, j);
