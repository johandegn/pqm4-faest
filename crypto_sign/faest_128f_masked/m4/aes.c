...
static bf8_t affine_incomplete_table[256] = {
...
// ## AES ##
// Round Functions
void __attribute__ ((noinline))  add_round_key(unsigned int round, aes_block_t state, const aes_round_keys_t* round_key,
...
void __attribute__ ((noinline)) store_state(uint8_t* dst, aes_block_t state, unsigned int block_words) {
  for (unsigned int i = 0; i != block_words * 4; ++i) {
