...
int crypto_sign_keypair(unsigned char* pk, unsigned char* sk) {
  return faest_128f_keygen(pk, sk);
}

int crypto_sign(unsigned char* sm, size_t* smlen, const unsigned char* m,
                size_t mlen, const unsigned char* sk) {
  *smlen = mlen + FAEST_128F_SIGNATURE_SIZE;
  memmove(sm, m, mlen);

  size_t signature_len = FAEST_128F_SIGNATURE_SIZE;
  return faest_128f_sign(sk, sm, mlen, sm + mlen, &signature_len);
}

int crypto_sign_open(unsigned char* m, size_t* mlen, const unsigned char* sm,
                     size_t smlen, const unsigned char* pk) {
  if (smlen < FAEST_128F_SIGNATURE_SIZE) {
    // signature too short
    return -1;
  }
  size_t m_length = smlen - FAEST_128F_SIGNATURE_SIZE;
  if (faest_128f_verify(pk, sm, m_length, sm + m_length, FAEST_128F_SIGNATURE_SIZE)) {
    return -1;
  }
